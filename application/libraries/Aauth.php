...
class Aauth {
	...
	public function __construct() {
		...
		// Dependancies
		if(CI_VERSION >= 2.2){ $this->CI->load->library('driver'); }
	...
	public function login($email, $pass, $remember = FALSE) {
		...
		*/
		...
		if ( $query->num_rows() > 0 )
		{
			// If email and pass matches create session
			$data = array('id' => $row->id,'name' => $row->name,'email' => $row->email,'loggedin' => TRUE);
	...
	public function remind_password($email)
	{
		...
		if ($query->num_rows() > 0)
		{
			...
			$this->CI->email->message('<b>Estimado '.$row->name.':<br><br></b> Recibimos una solicitud de cambio de contraseña para este correo<br> Para cambiar tu contraseña utiliza el siguiente enlace:<br>'.
			'<a href="http://localhost/easyCFDi/index.php/usuarios/reset_passwordn/'.$row->id . '/' . $ver_code. '">http://localhost/easyCFDi/index.php/usuarios/reset_password/'.$row->id . '/' . $ver_code. '</a>');
	...
	public function reset_password($user_id, $ver_code)
	{
		...
		if( $query->num_rows() > 0 )
		{
			$data =	 array('verification_code' => '', 'pass' => $this->hash_password($pass, $user_id));
			$row = $query->row();
			$email = $row->email;
			$this->CI->db->where('id', $user_id);
			$this->CI->db->update($this->config_vars['users'] , $data);
	...
	public function update_login_attempts($email)
	{
		...
		if ( $data['login_attempts'] > $this->config_vars['max_login_attempt'] ) { return FALSE; }
		 else { return TRUE;}
	}
	...
	public function update_user($user_id, $email = FALSE, $pass = FALSE, $name = FALSE)
	{
		...
		if ($pass != FALSE)  { $data['pass'] = $this->hash_password($pass, $user_id);}
	...
	public function list_users($group_par = FALSE, $limit = FALSE, $offset = FALSE, $include_banneds = FALSE) {
		...
		else {$this->CI->db->select('*')->from($this->config_vars['users']);}
		// banneds
		if (!$include_banneds) { $this->CI->db->where('banned != ', 1);}
		// limit
		if ($limit)
		{
			if ($offset == FALSE)
	...
	public function verify_user($user_id, $ver_code)
	{
		...
		if( $query->num_rows() > 0 )
		{
			$data =	 array('verification_code' => '','banned' => 0);
			$this->CI->db->where('id', $user_id);
			$this->CI->db->update($this->config_vars['users'] , $data);
			return TRUE;
		}
	 ...
	 */
	public function send_verification($user_id,$user,$pass,$razon)
	{
			$query = $this->CI->db->where( 'id', $user_id );
			$query = $this->CI->db->get( $this->config_vars['users'] );
			if ($query->num_rows() > 0)
			{
				...
				if ($pass=="0")
				{
					...
					$this->CI->email->message('<b>'.$row->name.'<br><br>'.$this->CI->lang->line('permisos_admin')."</b><br>".$razon."<br><br><b>".
					$this->CI->lang->line('correo_usuario')."</b><br>".$user);
				}
				else
				{
					$this->CI->email->message('<b>'.$row->name.':<br><br>'.$this->CI->lang->line('alta_permisos_admin').'</b><br>'.$razon."<br><br><b>".
					$this->CI->lang->line('correo_usuario')."</b><br>".$user.'<br><br><b>'.
				  $this->CI->lang->line('aauth_email_verification_code')."</b><br>".$ver_code.'<br><br><b>'.
					$this->CI->lang->line('aauth_email_verification_link').'</b><br>'.
					'<a href="http://localhost/easyCFDi/index.php/usuarios/verificacion/'.$user_id . '/' . $ver_code. '/'.$pass.'">http://localhost/easyCFDi/index.php/usuarios/verificacion/'.$user_id . '/' . $ver_code. '/'.$pass.'</a>');
			  }
				 $this->CI->email->set_mailtype("html");
			 $this->CI->email->send();
		}
	...
	public function delete_user($user_id)
	{
		...
		// delete user vars
		$this->CI->db->where('user_id', $user_id);
		if($this->CI->db->delete($this->config_vars['user_variables'])) {return true;}
		else {return false;}
	}
	...
	{
		$data = array('banned' => 1,'verification_code' => '');
		$this->CI->db->where('id', $user_id);
		return $this->CI->db->update($this->config_vars['users'], $data);
	}
	...
	{
		if( ! $email){ $query = $this->CI->db->where('id', $this->CI->session->userdata('id'));}
		else { $query = $this->CI->db->where('email', $email);}
		$query = $this->CI->db->get($this->config_vars['users']);
		if ($query->num_rows() <= 0)
		{
			$this->error($this->CI->lang->line('aauth_error_no_user'));
	...
	{
		$query = $this->CI->db->get_where($this->config_vars['groups'], array('name' => $group_name,'definition' => $definition));
		if ($query->num_rows() < 1)
		{
			$data = array( 'name' => $group_name, 'definition' => $definition );
	...
	public function create_perm($perm_name, $definition='')
	{
		...
		if ($query->num_rows() < 1)
		{
			$data = array( 'name' => $perm_name, 'definition'=> $definition);
	...
	public function is_allowed($perm_par, $user_id=FALSE)
	{
		...
		if( $query->num_rows() > 0){ return TRUE;}
		elseif ($this->is_group_allowed($perm_id)) {return TRUE;}
		else {return FALSE;}
	}
	...
	public function is_group_allowed($perm_par, $group_par=FALSE)
	{
		...
		// checks current user's all groups
			...
			{ if($this->is_group_allowed($perm_id, $g -> id)){return TRUE;}}
			return FALSE;
		}
	}
	...
	public function send_pm( $sender_id, $receiver_id, $title, $message )
	{
		...
		$data = array('sender_id' => $sender_id,'receiver_id' => $receiver_id,'title' => $title,'message' => $message,'date' => date('Y-m-d H:i:s'));
	...
	{
		$query='';
		if ( $receiver_id != FALSE){$query = $this->CI->db->where('receiver_id', $receiver_id);}
		if( $sender_id != FALSE ){ $query = $this->CI->db->where('sender_id', $sender_id);}
	...
	public function get_pm($pm_id, $set_as_read = TRUE)
	{
		...
		if ($query->num_rows() < 1) { $this->error( $this->CI->lang->line('aauth_error_no_pm') );}
   ...
   return $this->CI->db->delete( $this->config_vars['pms'], array('id' => $pm_id) );
	...
	{
		if(!$receiver_id){ $receiver_id = $this->CI->session->userdata('id');}
	...
	public function keep_errors($include_non_flash = FALSE)
	{
		...
		if($include_non_flash) { $this->flash_errors = array_merge($this->flash_errors, $this->errors);}
	...
	public function set_user_var( $key, $value, $user_id = FALSE )
	{
		...
		if ( ! $this->get_user($user_id)){return FALSE;}
		// if var not set, set
		 if ($this->get_user_var($key,$user_id) ===FALSE)
		 {
			$data = array( 'key' => $key, 'value' => $value, 'user_id' => $user_id);
	...
	public function list_user_var_keys($user_id = FALSE)
	{
		...
		if ($query->num_rows() < 1) { return FALSE;}
			...
			$key_list = array();
			foreach( $query->result() as $row) { $key_list[] = $row->key;}
			return $key_list;
		}
	}
