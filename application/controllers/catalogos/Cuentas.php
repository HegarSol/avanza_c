...
class Cuentas extends MY_Controller
{
    ...
    public function ajax_cuentasDiario()
    {
        $list = $this->cuentas->get_datatables();
        $data = array();
        foreach($list as $cuentas)
        {
            $row = array();
            $row[] = '<button type="button" class="btn btn-primary" onclick="seleccionarcunetadiario(\''.$cuentas->cuenta.'\',\''.$cuentas->sub_cta.'\',\''.$cuentas->nombre.'\',\''.$cuentas->ssub_cta.'\')">Seleccionar</button>';
            // $row[] = $cuentas->idcuenta;
            $row[] = $cuentas->cuenta;
            $row[] = $cuentas->sub_cta;
            $row[] = $cuentas->nombre;
